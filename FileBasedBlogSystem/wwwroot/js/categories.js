...
function displayPosts(posts) {
  if (!posts.length) {
    postsContainer.innerHTML = `<p>No posts in this category.</p>`;
    return;
  }
...
async function loadCategoryPosts() {
  const { posts, totalPages } = await fetchCategoryPosts();
  displayPosts(posts);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!category) {
    postsContainer.innerHTML = `<p>No category specified.</p>`;
    return;
  }
